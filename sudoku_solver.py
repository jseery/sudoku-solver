...
def setup_puzzle(size):
	...
	Puzzle = namedtuple('Puzzle', field_names='squares, unitlist, units, peers')

	return Puzzle(squares=squares, unitlist=unitlist, units=units, peers=peers)


def test_puzzle(Puzzle):
    "A set of unit tests."
    assert len(Puzzle.squares) == 81
